...
exports.init = function init(mongodbPort,mongoDBName,callback) {
  mongodbServer = new mongodb.Server('localhost', mongodbPort, { auto_reconnect: true, poolSize: 10 });
      ...
      if (err == null) {
        collection.count((err, data) => {
          if (err) {
            ...
            callback(false, 'get collection count error');
          } else {
            callback(true, data);
          }
        });
      ...
      if (err == null) {
        collection.insert(obj, (err, data) => {
          ...
          if (err!=null) {
            ...
            callback(false, 'insert ' + obj + ' error');
          } else {
            callback(true, data);
          }
        });
...
exports.queryFindAll = function queryFindAll(collectionName, obj, callback) {
  if (controlDB != null) {
      ...
      if (err == null) {
        collection.find(obj, function(err, cursor) {
          ...
          if (!err) {
            ...
            console.log(cursor);
            console.log(obj);
            callback(false, 'query ' + obj + ' error');
          }
        });
      } else {
        console.log('\x1b[31m mongodb ' + collectionName + ' open error \x1b[37m');
...
exports.update = function update(collectionName, obj, updateData, callback) {
  if (controlDB != null) {
    ...
    controlDB.collection(collectionName, function(err, collection) {
      if (err == null) {
        collection.update(obj, updateData, function(err, result) {
          if (err) {
            console.log('\x1b[31m update ' + obj + ' error \x1b[37m');
            console.log(err);
            console.log(result);
            console.log(obj);
            callback(false, 'update ' + obj + ' error');
          } else {
      ...
      if (err == null) {
        collection.remove(obj, function(err, result) {
          if (err) {
            ...
            console.log(result);
            console.log(obj);
            callback(false, 'remove ' + obj + ' error');
          } else {
            callback(true, result.result);
          }
        });
